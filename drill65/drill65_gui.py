...
def load_gui(self):
    ...
    self.btn_srctag = tk.Button(self.master,text="Choose Source",command =lambda: drill65_func.select_source(self))
    self.btn_srctag.grid(row=0,column=5,rowspan=1,columnspan=2,padx=(30,40),pady=(10,0),sticky=N+E)
    
    self.btn_desttag = tk.Button(self.master,text="Choose Destination",command =lambda: drill65_func.select_dest(self))
    self.btn_desttag.grid(row=1,column=5,rowspan=1,columnspan=2,padx=(30,40),pady=(10,0),sticky=N+E)


    self.btn_submit = tk.Button(self.master, text = "BACKUP",height = '4',width = '18',bd='3px',command =lambda: drill65_func.backup(self))
    self.btn_submit.grid(row = 3, column = 2,rowspan=2,columnspan=2,padx=(30,40),pady=(25,0),sticky=N+W)
    self.btn_submit = tk.Button(self.master, text = "CANCEL",height = '4',width = '15',bd='3px',command =lambda: drill65_func.clear(self))
